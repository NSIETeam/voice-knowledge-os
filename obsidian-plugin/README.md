...
The plugin reads `voice_memory_id` from note frontmatter, so it does not require a fixed `Recordings/` directory. API and Ollama URLs are configurable but are restricted to localhost/loopback. Recompile uses the configured local Ollama model and shows the resulting Markdown diff; the current API writes the managed block before showing that diff and retains rollback history. Do not treat this as an approve-before-write workflow. Speaker confirmation applies only to the current record; persistent voice profiles are not implemented.
...
- Recompile a processing profile with local Ollama and inspect the generated diff.
