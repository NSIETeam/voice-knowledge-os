...
The repository includes local whisper.cpp transcription, an optional NeMo-Speech.cpp diarization adapter, and an optional Ollama semantic-processor adapter. It does not bundle model runtimes or weights. Diarization clusters are local, per-record suggestions requiring human review; persistent voice identity remains unimplemented. Automatic model downloads are disabled in the desktop integration: users must select an existing local diarization model and review its separate license.

The NeMo adapter invokes the local `nemo-speech diarize --model <local-model.gguf> --format rttm` command, reads RTTM speaker intervals, and aligns them to the existing transcript without replacing its text or timestamps. See the upstream [CLI guide](https://github.com/NVIDIA/NeMo-Speech.cpp/blob/main/docs/cli.md) and [model notes](https://github.com/NVIDIA/NeMo-Speech.cpp/blob/main/docs/asr/models.md); runtime code and model weights are not bundled, and their licenses are separate.
