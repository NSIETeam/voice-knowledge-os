...
The local API and content-addressed audio ledger are now included. Start the API with `PYTHONPATH=src python -m voice_memory.cli serve .voice-memory`; it exposes a local status page, `GET /health`, `GET /profiles`, `POST /ledger/import`, and `POST /records/compile`. The API binds to loopback by default and stores imported bytes under a hash-addressed object path.
